"""v9: fine-tune (unfreeze) the GloVe embeddings, isolated from v8's other
choices - same 5-fold CV, same mean-pool architecture, same features.

Only change: embeddings are trainable (freeze=False) instead of fixed. To
avoid catastrophic forgetting of the pretrained structure, the embedding
layer trains at a lower learning rate (1e-4) than the dense head (1e-3) via
separate Adam param groups - the head can move fast since it starts random,
the embeddings should only drift a little from their pretrained values.
"""
...
class MeanPoolClassifier(nn.Module):
    def __init__(self, embedding_matrix: np.ndarray, numeric_dim: int = 3, hidden: int = 64, dropout: float = 0.3):
        super().__init__()
        # freeze=False vs v7/v8: embeddings are now fine-tuned, not fixed
...
def build_optimizer(model: MeanPoolClassifier) -> torch.optim.Optimizer:
...
def main():
    ...
    submission_path = Path(__file__).parent / "submission_v9.csv"
