"""v10: bidirectional LSTM pooling instead of mean pooling.

Builds on v9 (fine-tuned GloVe embeddings, 5-fold CV, numeric features,
OOF threshold tuning, fold-averaged test predictions) - all unchanged.
Only the sequence -> fixed-vector step changes:

Mean pooling (v7-v9) treats a tweet as an unordered bag of embeddings -
"crash car the" and "the car crash" pool to the same vector. A
bidirectional LSTM instead reads the token sequence in order (forward and
backward), so word order and short-range dependencies ("not on fire" vs
"on fire") can actually influence the representation.

Uses nn.utils.rnn.pack_padded_sequence/pad the LSTM only processes real
tokens per sequence and skips the padding positions entirely, rather than
running the recurrence over meaningless zero-vector pad steps.
"""
...
class TweetDataset(Dataset):
    def __init__(self, df: pd.DataFrame, vocab: dict, has_target: bool):
        ...
        # real (unpadded) length per row, min-clamped to 1 for empty-after-cleaning tweets
        self.lengths = np.clip((self.token_ids != 0).sum(axis=1), 1, None).astype(np.int64)
...
class LSTMClassifier(nn.Module):
    def __init__(self, embedding_matrix: np.ndarray, numeric_dim: int = 3, hidden: int = 64, dropout: float = 0.3):
        ...
        self.fc1 = nn.Linear(LSTM_HIDDEN * 2 + numeric_dim, hidden)
        self.dropout = nn.Dropout(dropout)
        self.fc2 = nn.Linear(hidden, 1)

    def forward(self, token_ids, lengths, numeric_feats):
        embedded = self.embedding(token_ids)  # (batch, seq_len, embed_dim)
        packed = pack_padded_sequence(
            embedded, lengths.cpu(), batch_first=True, enforce_sorted=False
        )
        _, (h_n, _) = self.lstm(packed)
        # h_n: (num_directions, batch, hidden) for a single-layer LSTM -
        # h_n[0] is the final forward-direction state, h_n[1] the final
        # backward-direction state (i.e. the state after reading the whole
        # sequence right-to-left, evaluated at token 0)
...
def predict_proba(model, loader) -> np.ndarray:
    ...
    with torch.no_grad():
        for batch in loader:
            token_ids, lengths, numeric = batch[0].to(DEVICE), batch[1], batch[2].to(DEVICE)
            logits = model(token_ids, lengths, numeric)
            probs.append(torch.sigmoid(logits).cpu().numpy())
    return np.concatenate(probs)


def build_optimizer(model: LSTMClassifier) -> torch.optim.Optimizer:
    head_params = [p for name, p in model.named_parameters() if not name.startswith("embedding")]
    return torch.optim.Adam(
        [
            {"params": model.embedding.parameters(), "lr": EMBED_LEARNING_RATE},
            {"params": head_params, "lr": HEAD_LEARNING_RATE},
        ]
    )


def train_one_fold(train_split, val_split, embedding_matrix, vocab, test_loader, fold_idx):
    ...
    for epoch in range(1, EPOCHS + 1):
        model.train()
        for token_ids, lengths, numeric, target in train_loader:
            token_ids, numeric, target = token_ids.to(DEVICE), numeric.to(DEVICE), target.to(DEVICE)
            optimizer.zero_grad()
            loss = criterion(model(token_ids, lengths, numeric), target)
            loss.backward()
            optimizer.step()
...
def main():
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(train_df, y), start=1):
        ...
        fold_val_proba, fold_test_proba = train_one_fold(
            train_split, val_split, embedding_matrix, vocab, test_loader, fold_idx
        )
    ...
    submission_path = Path(__file__).parent / "submission_v10.csv"
